#!/usr/bin/env python3
"""Arven Solé — Teninde Kal TikTok (latest Short language)."""
...
OUT_TIKTOK = Path("/workspace/ArvenSole_TenindeKal_TIKTOK.mp4")
...
# Hottest chorus 2 — Teninle kal bu gece (never Seninle)
SHORT_T0 = 120.7
SHORT_T1 = 154.5
WORD_LEAD = 0.05
...
def _display_word(raw: str) -> str:
    ...
    if key in {"seninle", "seninde", "teninle"}:
        return "TENİNLE"
...
def render_tiktok() -> None:
    ...
    audio_clip = Path("/tmp/teninde_tiktok_clip.wav")
    subprocess.check_call(
        [
            "ffmpeg", "-y",
