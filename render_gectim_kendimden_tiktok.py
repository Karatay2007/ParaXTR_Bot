#!/usr/bin/env python3
"""Arven Solé — Geçtim Kendimden TikTok (9:16).

Same language as latest YouTube Short (Takılsın):
DEVAMI YAYINDA, kinetic uppercase lyrics, open EQ, credit stack,
like / ABONE OL / bell cursor CTA. Native render (not a YT re-export).
"""
...
OUT_TIKTOK = Path("/workspace/ArvenSole_GectimKendimden_TIKTOK.mp4")
...
# Hottest stretch: pre-chorus build → full chorus 2 climax (longer OK for TikTok)
SHORT_T0 = 116.2
SHORT_T1 = 168.5
WORD_LEAD = 0.05

# Cleaned word clocks. Whisper fixes: yandığı→Yandı, yer desin→yerdesin,
# bitmedim→bitmedin, duymuyor musun→Duymuyor musun
LINES: list[list[tuple[float, float, str]]] = [
    # Pre-chorus build
    [(116.42, 117.82, "Nefesim"), (117.82, 118.60, "daralıyor"), (118.60, 119.10, "her"), (119.10, 119.44, "an")],
    [(119.70, 119.96, "Seni"), (119.96, 121.14, "düşününce"), (121.14, 122.16, "hızlanıyor"), (122.16, 122.86, "kan")],
    # Chorus 2
    [(124.26, 125.66, "Geçtim"), (125.66, 126.96, "kendimden")],
    [(127.14, 129.36, "Geçemedim"), (129.36, 130.54, "senden")],
    [(130.54, 131.36, "Yandı"), (131.36, 133.24, "içim")],
    [(133.24, 134.04, "Sönmedi"), (134.04, 135.52, "ateşin")],
    [(137.74, 139.14, "Geçtim"), (139.14, 140.38, "kendimden")],
    [(140.52, 142.80, "Geçemedim"), (142.80, 143.82, "senden")],
    [(143.82, 144.26, "Kalp"), (144.26, 145.54, "çarpıyor")],
    [(146.94, 149.52, "Her yerdesin")],
    [(150.46, 151.66, "Bağırıyorum")],
    [(152.00, 152.68, "Duymuyor"), (152.68, 153.18, "musun")],
    [(153.18, 153.88, "Bu"), (153.88, 154.18, "gece"), (154.18, 154.60, "yine"), (154.60, 155.88, "sensin")],
    [(156.80, 157.50, "Geçtim"), (157.50, 159.08, "kendimden")],
    [(160.12, 161.30, "Geçemedim"), (161.30, 162.36, "senden")],
    [(162.36, 163.14, "Bitti"), (163.14, 164.14, "sandım"), (164.14, 165.20, "bitmedin")],
]
...
def draw_kinetic_lyrics(img: Image.Image, t: float, audio_level: float = 0.35) -> None:
    ...
    fsize = max(46, int(52 * (1.0 + 0.035 * lvl)))
...
def render_tiktok() -> None:
    ...
    print(f"Rendering TIKTOK {t0:.1f}-{t1:.1f}s ({n_frames}f)", flush=True)

    audio_clip = Path("/tmp/gectim_tiktok_clip.wav")
    subprocess.check_call(
        [
            "ffmpeg", "-y",
    ...
    dest = ART / OUT_TIKTOK.name
    dest.write_bytes(OUT_TIKTOK.read_bytes())
    print(f"Done {OUT_TIKTOK} ({OUT_TIKTOK.stat().st_size / 1e6:.1f}MB)", flush=True)
