#!/usr/bin/env python3
"""Arven Solé — Derin Bir Sessizlik YouTube Short (latest Short language)."""
...
OUT_SHORT = Path("/workspace/ArvenSole_DerinBirSessizlik_SHORT.mp4")
...
# Hottest: verse tag → chorus 2 title hook
SHORT_T0 = 91.2
SHORT_T1 = 117.5
WORD_LEAD = 0.05
...
def base_vertical() -> Image.Image:
    ...
    center_spaced(TITLE, title_fnt, line_y + 110, CREAM, 5)
...
def render_short() -> None:
    ...
    audio_clip = Path("/tmp/derin_short_clip.wav")
    subprocess.check_call(
        [
            "ffmpeg", "-y",
