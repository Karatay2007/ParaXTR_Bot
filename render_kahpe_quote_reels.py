#!/usr/bin/env python3
"""Kahpe İnsan — Instagram Reels 9:16 quote Short.

Cut: Kim dost kim düşman → final chorus.
On-screen: user quote (betrayal / ingratitude). Real stock matching lyric theme.
No lyric UI / no 16:9.
"""
...
OUT = Path("/workspace/ArvenSole_KahpeInsan_REELS_QUOTE.mp4")
ART = Path("/opt/cursor/artifacts")
STOCK = Path("/tmp/kahpe_quote")
WORK = Path("/tmp/kahpe_reels_quote")
...
W, H = 1080, 1920
FPS = 24
# Same hottest cut as lyric Short: Kim dost → chorus end
T0 = 198.5
T1 = 236.2

QUOTE = (
    "Bu dünyadaki en ağır yük, iyi niyet ve iyiliktir. "
    "Çünkü sonu hep ihanet ve nankörlükle ödenir."
)

# Lyric-matched beds: distance / who is friend / fight / turn away / pain
SEGMENTS = [
    (STOCK / "42971.mp4", 0.4, 9.5),   # sofa distance — kim dost kim düşman
    (STOCK / "4574.mp4", 1.0, 10.0),    # back-to-back — dost gibi / mesafe
    (STOCK / "4504.mp4", 1.5, 8.5),     # argument — ihanet gerilimi
    (STOCK / "4714.mp4", 0.3, 8.0),     # looking away — sırt çevirmek
    (STOCK / "42253.mp4", 0.5, 8.0),    # scream / broken trust
]
...
def to_vertical(src: Path, t0: float, dur: float, dest: Path) -> None:
    vf = (
        ...
        f"crop={W}:{H},"
        "eq=brightness=0.10:saturation=1.02:contrast=1.04,"
        "vignette=PI/8"
    )
...
def make_quote_overlay() -> Image.Image:
    ...
    for y in range(top, bot):
        ...
        a = int(145 * max(0.0, 1.0 - p**1.25))
        wd.line([(40, y), (W - 40, y)], fill=(0, 0, 0, a))
    layer = Image.alpha_composite(layer, wash)

    d = ImageDraw.Draw(layer)
    fnt = _font(52)
    lines = wrap_quote(d, fnt, max_w=W - 140)
    line_gap = 14
    heights = []
    widths = []
    for line in lines:
    ...
    rule_w = 64
    d.rectangle(
        ((W - rule_w) // 2, y - 40, (W + rule_w) // 2, y - 38),
        fill=(232, 214, 170, 230),
    )

    cream = (248, 240, 226, 255)
    accent = (255, 196, 160, 255)  # soft warm on "ihanet"
    for i, line in enumerate(lines):
        tw = widths[i]
        x = (W - tw) // 2
        for ox, oy in ((2, 2), (3, 3), (1, 1)):
            d.text((x + ox, y + oy), line, font=fnt, fill=(0, 0, 0, 175))
        low = line.lower()
        key = "ihanet"
        if key in low:
            idx = low.find(key)
            before, mid, after = line[:idx], line[idx : idx + len(key)], line[idx + len(key) :]
            cx = x
            if before:
    ...
    d.rectangle(
        ((W - rule_w) // 2, y + 22, (W + rule_w) // 2, y + 24),
        fill=(232, 214, 170, 230),
    )
    bloom = layer.filter(ImageFilter.GaussianBlur(0.7))
...
def render() -> None:
    ...
    clip_dur = T1 - T0
    print(f"Reels 9:16  {T0:.2f} → {T1:.2f} ({clip_dur:.2f}s)", flush=True)
