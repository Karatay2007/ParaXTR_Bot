#!/usr/bin/env python3
"""Geçemedim Senden — YouTube Shorts quote (ferah bed + hook text + ABONE CTA).

Cut: chorus ~64→95 (Geçemedim senden / kaldım geride).
On-screen quote matches user photo; last line emphasized.
Subscribe CTA below quote with breathing room (not tight).
"""
...
# Photo lines — hook: denial stack → turn → punchline
HOOK_LINES = [
    "Vazgeçtim diyorum.",
    "Bitti diyorum.",
    "Olmayacak diyorum.",
...
BRIDGE_LINES = [
    "Ama içimdeki umudu öldüremiyorum.",
]
PUNCH_LINES = [
...
def to_vertical(src: Path, t0: float, dur: float, dest: Path) -> None:
    vf = (
        ...
        f"crop={W}:{H},setsar=1,"
        "eq=brightness=0.13:saturation=1.08:contrast=1.02,"
        "vignette=PI/14"
    )
...
def make_quote_overlay() -> Image.Image:
    """Centered hook layout matching photo rhythm; punchline larger/bolder."""
    layer = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    d = ImageDraw.Draw(layer)

    # top credit
    cf = _font(FONT_DIR / "Inter-Bold.ttf", 26)
    credit = "ARVEN SOLÉ  ·  GEÇEMEDİM SENDEN"
    ...
    hook_fnt = _font(FONT_ITALIC, 48)
    bridge_fnt = _font(FONT_ITALIC, 46)
    punch_fnt = _font(FONT_BOLD, 50)  # upright semi-bold punch (photo last line emphasis)
    # Prefer italic bold if available — SemiBold upright + slight size bump
    punch_italic = _font(FONT_ITALIC, 52)

    def tw(text: str, fnt) -> int:
        b = d.textbbox((0, 0), text, font=fnt)
        return b[2] - b[0]

    # measure block so it sits upper-mid with clear air above CTA
    items: list[tuple[str, ImageFont.FreeTypeFont, int]] = []
    for line in HOOK_LINES:
        items.append((line, hook_fnt, 10))
    items.append(("", hook_fnt, 36))  # photo gap
    for line in BRIDGE_LINES:
        items.append((line, bridge_fnt, 14))
    for line in PUNCH_LINES:
        items.append((line, punch_italic, 8))
    ...
    y = max(220, ((CTA_Y - 160) - text_h) // 2 - 40)
    # keep quote clearly above CTA
    y = min(y, 320)
    ...
    # thin divider under punch — breathing before CTA
    div_y = y + 28
    div_w = 64
    d = ImageDraw.Draw(layer)
    d.line(
...
def render() -> None:
    ...
    clip_dur = T1 - T0
    print(f"Geçemedim quote Short  {T0:.2f}→{T1:.2f} ({clip_dur:.2f}s)", flush=True)
    ...
    try:
        for fi in range(n_frames):
            ...
            # CTA after text settled — clear gap under quote
            if t >= 2.0:
