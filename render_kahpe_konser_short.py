#!/usr/bin/env python3
"""Arven Solé — Kahpe Konser YouTube Short (latest Short language, no DEVAMI)."""
...
AUDIO = Path("/workspace/arven_sole_tracks/Kahpe_Konser.mp3")
COVER = Path("/opt/cursor/artifacts/assets/arven-sole-cover.png")
WAV = Path("/tmp/kahpe_konser.wav")
...
OUT_SHORT = Path("/workspace/ArvenSole_KahpeKonser_SHORT.mp4")
...
# Bridge → final chorus (concert cut). No DEVAMI.
SHORT_T0 = 196.0
SHORT_T1 = 235.5
WORD_LEAD = 0.05

LINES: list[list[tuple[float, float, str]]] = [
    [(196.34, 197.50, "Kim"), (197.50, 197.84, "dost"), (197.84, 198.26, "kim"), (198.26, 199.00, "düşman")],
    [(199.00, 199.58, "Çizgi"), (199.58, 200.14, "duman")],
    [(200.14, 200.98, "İsim"), (200.98, 201.58, "değişir")],
    [(201.58, 202.08, "Huy"), (202.08, 202.60, "aynı"), (202.60, 203.08, "kalır")],
    [(203.08, 203.64, "elini"), (203.64, 204.44, "sıkarken"), (204.44, 205.30, "aklın"), (205.30, 206.14, "başka")],
    [(206.14, 206.68, "Ben"), (206.68, 207.54, "yanarken"), (207.54, 207.92, "sen"), (207.92, 211.30, "hesapta")],
    [(211.30, 212.26, "Kahpe"), (212.26, 213.06, "insan")],
    [(213.44, 214.00, "tanıdım"), (214.00, 214.82, "seni")],
    [(214.82, 215.58, "Gözün"), (215.58, 216.08, "güler"), (216.32, 216.52, "için"), (216.52, 216.98, "yer"), (216.98, 217.30, "beni")],
    [(217.30, 218.26, "Kahpe"), (218.26, 218.96, "insan")],
    [(219.28, 219.70, "bırakıp"), (219.70, 220.26, "gittin")],
    [(220.26, 220.64, "Dost"), (220.64, 221.04, "gibi"), (221.04, 221.64, "durdun"), (221.72, 222.62, "ihanet"), (222.62, 223.18, "ettin")],
    [(223.18, 224.14, "Kahpe"), (224.14, 224.76, "insan")],
    [(225.10, 225.96, "utanmadın"), (225.96, 226.10, "mı")],
    [(226.10, 226.88, "Sırtımdan"), (226.88, 227.66, "vurdun"), (227.98, 228.42, "dönüp"), (228.42, 229.06, "bakmadın")],
    [(229.06, 230.04, "Kahpe"), (230.04, 230.60, "insan")],
    [(230.96, 231.36, "yazdım"), (231.36, 231.96, "adını")],
    [(231.96, 232.62, "Unutmam"), (232.62, 233.22, "seni")],
    [(233.70, 234.16, "silmem"), (234.16, 234.92, "acını")],
]
...
def base_vertical() -> Image.Image:
    ...
    center_spaced(TITLE, title_fnt, line_y + 110, CREAM, 6)
...
def render_short() -> None:
    ...
    audio_clip = Path("/tmp/kahpe_konser_short_clip.wav")
    subprocess.check_call(
        [
            "ffmpeg", "-y",
